...
<head>
    <meta charset="UTF-8">
    <link rel="stylesheet" href="newPageMan.css">
...
<body>
    ...
    $id = $_GET['id'];
    $row = $pdo->query("SELECT * FROM sneakersforman WHERE id = $id");
    $row = $row->fetch();
                ...
                <div class="this-page__dscr-size">
                    <div class="this_page__size">
                        ...
                        <ul class="this-page__size-items">
                            <?php
                            $size = $pdo->query("SELECT * FROM man_size WHERE sneakersman_id = $id");
                            $size = $size->fetch();
                            if ($size['size_34'] != '') {
